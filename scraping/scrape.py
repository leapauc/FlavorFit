...
def scraper_recettes(url):
    ...
    data = []
    while requests.get(f'{url}{compteur_page}', headers=headers).status_code == 200 and compteur_page<=100 :
        print(compteur_page)
        response = requests.get(f'{url}{compteur_page}', headers=headers)
...
def tab_file_recettes(donnees):
    ...
    df.to_csv('scraping/recettes_scrapees.csv', index=False, encoding='utf-8')
